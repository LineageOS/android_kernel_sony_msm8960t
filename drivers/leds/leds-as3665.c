...
#define AS3665_EFFECTS_SAVE_LOAD_LENGTH          180
...
struct as3665_effects {
	u32		lights_effects;
	char		save_load[AS3665_EFFECTS_SAVE_LOAD_LENGTH];
	u8		save_load_len;
				     ...
				     const char *buf, size_t len)
{
	...
	if (len < AS3665_EFFECTS_SAVE_LOAD_LENGTH) {
				     ...
				     const char *buf, size_t len)
{
	...
	if (AS3665_EFFECTS_MUSIC_MODE_MUSIC(effects)) {
		...
		for (i = 0; i < chip->num_leds; ++i) {
			...
			led_current = ((unsigned int)led_current * led->effects_current) / 0xFF;
