...
#ifdef CONFIG_FB
static void as3676_fb_suspend(struct as3676_data *data)
{
	...
	for (i = 0; i < 6; i++) {
		...
		if (led->pled->startup_current_uA && led->pled->fb_backlight) {
			as3676_set_brightness(data, led, 0);
		}
	}
...
static void as3676_fb_resume(struct as3676_data *data)
{
	struct device *dev = &data->client->dev;
	int i;
	...
	for (i = 0; i < 6; i++) {
		struct as3676_led *led = &data->leds[i];
		if (led->pled->startup_current_uA && led->pled->fb_backlight) {
			as3676_set_brightness(data, led, led->ldev.brightness);
		}
	}
...
static int as3676_fb_notifier_callback(struct notifier_block *self,
		unsigned long event, void *fbdata)
{
	struct fb_event *evdata = fbdata;
	int *blank;
	struct as3676_data *data = container_of(self,
			struct as3676_data, fb_notifier);

	if (evdata && evdata->data) {
		if (event == FB_EVENT_BLANK) {
			blank = evdata->data;
			if (*blank == FB_BLANK_UNBLANK)
				as3676_fb_resume(data);
			else if (*blank == FB_BLANK_POWERDOWN)
				as3676_fb_suspend(data);
		}
	}
...
void as3676_fb_backlight_setup(struct device *dev,
		struct as3676_data *data)
{
	data->fb_suspended = false;
	data->fb_notifier.notifier_call = as3676_fb_notifier_callback;
...
#ifdef CONFIG_FB
	as3676_fb_backlight_setup(&client->dev, data);
#endif
